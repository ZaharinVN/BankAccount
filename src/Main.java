class BankAccount {
    ...
    class DebitAccount extends BankAccount{  /* унаследуйте класс от BankAccount */
        ...
        @Override
        public void withdrawCash(int amount) {
            if (amount - this.amount >= 0){ // если на счету достаточно денег
                ...
                System.out.printf("У вас недостаточно средств для снятия суммы " + amount + " " + currency);    // иначе вывести сообщение "У вас недостаточно средств для снятия суммы {amount} {currency}"
            }
        }
        @Override
        public void showBalance() {
    ...
    class CreditAccount extends BankAccount { // создайте класс CreditAccount, который будет наследоваться от класса BankAccount
        ...
        CreditAccount (final int amount, final String currency, final int creditLimit){ // создайте конструктор, который принимает на вход переменные (int amount, String currency, int creditLimit) и присваивает их значения переменным класса
            super.amount = amount;
